/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/GeoServices.framework/GeoServices
 */

@interface GEOPDPlaceQuestionnaireResult : PBCodable <NSCopying> {
    bool  _alwaysPositionInitialRatingCtaTowardsTop;
    struct { 
        unsigned int has_status : 1; 
        unsigned int has_alwaysPositionInitialRatingCtaTowardsTop : 1; 
        unsigned int has_isOverride : 1; 
        unsigned int read_historicalMapsIds : 1; 
        unsigned int read_mapsId : 1; 
        unsigned int read_scorecardLayout : 1; 
        unsigned int wrote_anyField : 1; 
    }  _flags;
    NSMutableArray * _historicalMapsIds;
    bool  _isOverride;
    GEOPDMapsIdentifier * _mapsId;
    PBDataReader * _reader;
    struct os_unfair_lock_s { 
        unsigned int _os_unfair_lock_opaque; 
    }  _readerLock;
    unsigned int  _readerMarkLength;
    unsigned int  _readerMarkPos;
    GEOPDScorecardLayout * _scorecardLayout;
    int  _status;
}

@property (nonatomic) bool alwaysPositionInitialRatingCtaTowardsTop;
@property (nonatomic) bool hasAlwaysPositionInitialRatingCtaTowardsTop;
@property (nonatomic) bool hasIsOverride;
@property (nonatomic, readonly) bool hasMapsId;
@property (nonatomic, readonly) bool hasScorecardLayout;
@property (nonatomic) bool hasStatus;
@property (nonatomic, retain) NSMutableArray *historicalMapsIds;
@property (nonatomic) bool isOverride;
@property (nonatomic, retain) GEOPDMapsIdentifier *mapsId;
@property (nonatomic, retain) GEOPDScorecardLayout *scorecardLayout;
@property (nonatomic) int status;

+ (Class)historicalMapsIdType;
+ (bool)isValid:(id)arg1;

- (void).cxx_destruct;
- (int)StringAsStatus:(id)arg1;
- (void)addHistoricalMapsId:(id)arg1;
- (bool)alwaysPositionInitialRatingCtaTowardsTop;
- (void)clearHistoricalMapsIds;
- (void)copyTo:(id)arg1;
- (id)copyWithZone:(struct _NSZone { }*)arg1;
- (id)description;
- (id)dictionaryRepresentation;
- (bool)hasAlwaysPositionInitialRatingCtaTowardsTop;
- (bool)hasIsOverride;
- (bool)hasMapsId;
- (bool)hasScorecardLayout;
- (bool)hasStatus;
- (unsigned long long)hash;
- (id)historicalMapsIdAtIndex:(unsigned long long)arg1;
- (id)historicalMapsIds;
- (unsigned long long)historicalMapsIdsCount;
- (id)init;
- (id)initWithData:(id)arg1;
- (id)initWithDictionary:(id)arg1;
- (id)initWithJSON:(id)arg1;
- (bool)isEqual:(id)arg1;
- (bool)isOverride;
- (id)jsonRepresentation;
- (id)mapsId;
- (void)mergeFrom:(id)arg1;
- (void)readAll:(bool)arg1;
- (bool)readFrom:(id)arg1;
- (id)scorecardLayout;
- (void)setAlwaysPositionInitialRatingCtaTowardsTop:(bool)arg1;
- (void)setHasAlwaysPositionInitialRatingCtaTowardsTop:(bool)arg1;
- (void)setHasIsOverride:(bool)arg1;
- (void)setHasStatus:(bool)arg1;
- (void)setHistoricalMapsIds:(id)arg1;
- (void)setIsOverride:(bool)arg1;
- (void)setMapsId:(id)arg1;
- (void)setScorecardLayout:(id)arg1;
- (void)setStatus:(int)arg1;
- (int)status;
- (id)statusAsString:(int)arg1;
- (void)writeTo:(id)arg1;

@end
